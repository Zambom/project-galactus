...
vec3 scalarField(float x, float y, float z) {
    ...
    vec3 water = uWaterColor * (0.4 + c * 0.5);
    vec3 land = uLandColor * (c * 0.8);
    vec3 peak = uPeakColor * c;
...
void main()
{
    ...
    vec3 sphericalCoord = getSphericalCoord(uType, x * 1024.0, y * 1024.0, z * 1024.0);
